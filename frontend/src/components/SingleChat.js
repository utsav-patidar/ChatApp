...
import VideoCallRing from "./VideoCall/VideoCallRingUi";
...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  const [isRingtoneTrue, setRingTone] = useState(false);
  ...
  useEffect(() => {
    ...
    socket.on("receive vc", (data) => {
      console.log(data, "data");
      if (data?.senderData?._id === user._id) {
        setRingTone(true);
  ...
  const vcSubmit = () => {
    ...
    setVideoCallOn(true);
    const senderData = getSenderFull(user, selectedChat?.users);
  ...
  return (
    <>
      {isRingtoneTrue && <VideoCallRing senderInfo={senderInfo} />}
      {selectedChat ? (
        <>
          <Text
              ...
              (!selectedChat.isGroupChat ? (
                <>
                  ...
                  <IconButton
                    ...
                    bg="red.500"
                    color="white"
                    onClick={() => setVideoCallModel(true)}
                  />
                </>
              ) : (
                <>
      ...
      />
      <ConfirmationModel
        isOpen={videoCallModel}
        setVideoCallModel={setVideoCallModel}
        user={
          selectedChat?.users ? getSender(user, selectedChat?.users) : "xyz"
        }
        vcStart={vcSubmit}
      />
    </>
